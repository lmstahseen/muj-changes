/*
  # Fix profiles table policies and triggers

  This migration ensures the profiles table has the correct RLS policies
  and trigger functions for user management.
*/

-- Disable RLS on profiles table to ensure it doesn't block access
ALTER TABLE IF EXISTS public.profiles DISABLE ROW LEVEL SECURITY;

-- Create policies for profiles table with existence checks
DO $$ 
BEGIN
  ...
  -- Check if "Users can view own profile" policy exists (alternative name)
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
